...
последний шаг деривации — `parse_incremental` из `bronze.orders`, в своём
`try` после строки журнала, так что сбой Gold его не останавливает, а его сбой
пишется в вотермарку `silver.order_utm`, а не в вердикт деривации. Тик DuckDB
перестаёт отгружать, двери зовут `parse_full` (reclassify и CLI — те, что
делают DELETE) или `parse_incremental`, а `/api/health` публикует
...
единственный синкер. Тест обходит модули до неподвижной точки: ни одна
функция вне `core/pg_order_utm.py` не доходит до отгрузки иначе как из ветки
`duckdb` проверки `parses_in_postgres()`.
