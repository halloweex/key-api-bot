...
const PERIODS: { value: Period; label: string; mobileLabel: string }[] = [
  ...
  { value: 'last_week', label: 'Last Week', mobileLabel: 'L.Week' },
  { value: 'month', label: 'This Month', mobileLabel: 'Month' },
  { value: 'last_month', label: 'Last Month', mobileLabel: 'L.Mon' },
]
    ...
    <div className="relative flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
      <div className="flex items-center gap-0.5 bg-slate-100/80 rounded-xl p-1 border border-slate-200/60 flex-shrink-0">
        ...
        <Button
          ...
          className={`${period === 'custom' ? 'shadow-sm' : ''} whitespace-nowrap text-xs px-2 sm:px-3 py-1.5 min-h-[36px] sm:min-h-[32px] rounded-lg flex items-center gap-1.5`}
        >
