    ...
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4">
      ...
      {!hasData ? (
        <div className="h-[280px] flex items-center justify-center text-sm text-slate-400">
          No momentum data (need 2 periods)
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-xs font-medium text-green-700 uppercase tracking-wide">Gainers</span>
            </div>
            {gainers.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">No gainers</p>
            ) : (
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-xs font-medium text-red-700 uppercase tracking-wide">Losers</span>
            </div>
            {losers.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">No losers</p>
            ) : (
              <div className="space-y-1.5">
                {losers.map((p, i) => (
